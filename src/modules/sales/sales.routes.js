...
function salesRouter({ db, env }) {
  ...
  router.post('/', requireRole('admin','operator'), async (req, res, next) => { try { const input = parse(saleSchema, req.body); const result = await service.mutateSale(db, { operation: 'create', input, userId: req.user.id }); await audit(db, req, { action: 'sale.create', entityType: 'sale', entityId: result.id, after: input }); res.redirect('/'); } catch (error) { if (error.status !== 422) return next(error); renderSales(db, env, req, res, { status: 422, query: { ...req.query, tab: 'form' }, draft: req.body, fieldErrors: error.fieldErrors || {}, formError: error.message }).catch(next); } });
...
function salesApiRouter({ db }) {
  ...
  router.post('/', requireRole('admin','operator'), async (req, res, next) => { try { const input = parse(saleSchema, req.body); const result = await service.mutateSale(db, { operation: 'create', input, userId: req.user.id }); await audit(db, req, { action: 'sale.create', entityType: 'sale', entityId: result.id, after: input }); res.status(201).json({ ok: true, data: { id: result.id }, events: result.events }); } catch (error) { next(error); } });
